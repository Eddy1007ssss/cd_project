...
class AdminSustainabilityReportPage extends StatefulWidget {
  ...
  @override
  State<AdminSustainabilityReportPage> createState() => _AdminSustainabilityReportPageState();
}

class _AdminSustainabilityReportPageState extends State<AdminSustainabilityReportPage> {
  final _client = Supabase.instance.client;
  late Future<_SustainabilityReportData> _report;
  ...
  Future<_SustainabilityReportData> _loadReport() async {
    ...
    for (final row in rows) {
      ...
      final dateValue = row['record_date'];
    ...
    recyclingRecords == 0 ? 0.0 : recyclingTotal / recyclingRecords;
  ...
  String _reportPeriod(_SustainabilityReportData data) {
    if (data.firstDate == null || data.lastDate == null) {
      return 'No data';
    }

    if (data.firstDate!.year == data.lastDate!.year &&
        data.firstDate!.month == data.lastDate!.month &&
        data.firstDate!.day == data.lastDate!.day) {
      return _formatDate(data.firstDate!);
    }

    return '${_formatDate(data.firstDate!)} - ${_formatDate(data.lastDate!)}';
  }
        ...
        build: (context) => [
          ...
          pw.Table(
            ...
            children: [
              ...
              _pdfTableRow('Carbon Offset', '${_formatNumber(data.carbonOffsetKg)} kg'),
              _pdfTableRow('Water Saved', '${_formatNumber(data.waterSavedL)} L'),
              _pdfTableRow('Energy Saved', '${_formatNumber(data.energySavedKwh)} kWh'),
              _pdfTableRow('Recycling Rate', '${data.recyclingRate.toStringAsFixed(1)}%'),
            ],
          ),
          ...
          pw.Container(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                pw.SizedBox(height: 6),
                pw.Text(
                  'Carbon offset, water saved and energy saved use the total recorded values.',
                  style: const pw.TextStyle(fontSize: 9, color: PdfColors.grey700),
                ),
                pw.SizedBox(height: 4),
                pw.Text(
                  'Recycling rate uses the average recorded recycling rate.',
                  style: const pw.TextStyle(fontSize: 9, color: PdfColors.grey700),
                ),
              ],
            ),
  ...
  Widget build(BuildContext context) {
    return TourFlowPage(
      ...
      child: FutureBuilder<_SustainabilityReportData>(
        future: _report,
              ...
              Container(
                ...
                child: const Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Carbon offset, water saved and energy saved use total recorded values. Recycling rate uses the average recorded rate.',
                        style: TextStyle(
